...
import com.Ejercicio6.repository.ClienteRepository;
import com.Ejercicio6.repository.DireccionRepository;
...
public class ClienteServiceImp implements ClienteServicio {
	...
	public void saveCliente(Cliente cliente) {
		...
		if (direccion != null) {
			direccion.setCliente(cliente); // Enlazamos dirección -> cliente
	...
	public Cliente updateCliente(Integer idCliente, Cliente cliente) {
		...
		if (nuevaDireccion != null) {
	        nuevaDireccion.setId(optCliente.getDireccion().getId());
	        nuevaDireccion.setCliente(optCliente);    // ⚠ Añadir esto
	        direccionRepository.save(nuevaDireccion);
	        optCliente.setDireccion(nuevaDireccion);
	    }
	...
	public Cliente actualizarDireccion(Integer id, Direccion direccion) {
	    ...
	    direccion.setCliente(optCliente);   // Importante para bidireccional
	    direccionRepository.save(direccion);
